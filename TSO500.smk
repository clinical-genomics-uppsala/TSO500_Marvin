...
def get_input():
    ...
    if config["DNA_Samples"] != "No DNA" :
        ...
        input_list.append(["Results/DNA/" + s + "/vcf/" + s + "-ensemble.final.no.introns.AD20.vcf" for s in config["DNA_Samples"]])
        input_list.append(["Results/DNA/" + s + "/vcf/" + s + "-ensemble.final.no.introns.AD20.vep.multiplebp.vcf" for s in config["DNA_Samples"]])
    ...
    if config["RNA_Samples"] != "No RNA" :
        ...
        input_list.append(["RNA_bam/" + s + "Aligned.sortedByCoord.out.bam" for s in config["RNA_Samples"]])
        input_list.append(["RNA_bam/" + s + "Aligned.sortedByCoord.out.bam.bai" for s in config["RNA_Samples"]])
